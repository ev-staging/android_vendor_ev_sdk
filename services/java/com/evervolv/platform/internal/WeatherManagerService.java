...
import evervolv.weather.WeatherManager;
import evervolv.weather.IWeatherManager;
import evervolv.weather.IRequestInfoListener;
import evervolv.weather.IWeatherServiceProviderChangeListener;
import evervolv.weather.RequestInfo;
import evervolv.weather.WeatherInfo;
import evervolv.weatherservice.IWeatherProviderService;
import evervolv.weatherservice.IWeatherProviderServiceClient;
import evervolv.weatherservice.ServiceRequestResult;
...
public class WeatherManagerService extends VendorService {
    ...
    private final IBinder mService = new IWeatherManager.Stub() {
        ...
        public String getActiveWeatherServiceProviderLabel() {
            ...
            final long identity = Binder.clearCallingIdentity();
            try {
                String enabledProviderService = EVSettings.Secure.getString(
                        mContext.getContentResolver(), EVSettings.Secure.WEATHER_PROVIDER_SERVICE);
    ...
    public WeatherManagerService(Context context) {
        super(context);
        mContext = context;
    }
    ...
    public String getFeatureDeclaration() {
        return ContextConstants.Features.WEATHER_SERVICES;
    }

    @Override
    public void onStart() {
        publishBinderService(ContextConstants.WEATHER_SERVICE, mService);
    ...
    private void bindActiveWeatherProviderService() {
        String activeProviderService = EVSettings.Secure.getString(mContext.getContentResolver(),
                EVSettings.Secure.WEATHER_PROVIDER_SERVICE);
            ...
            public void onPackageModified(String packageName) {
                ...
                if (cn.getPackageName().equals(packageName) && !mIsWeatherProviderServiceBound) {
                    ...
                    if (!getContext().bindServiceAsUser(new Intent().setComponent(cn),
                            mWeatherServiceProviderConnection, Context.BIND_AUTO_CREATE,
                            UserHandle.CURRENT)) {
                        EVSettings.Secure.putStringForUser( mContext.getContentResolver(),
                                EVSettings.Secure.WEATHER_PROVIDER_SERVICE, null,
                                getChangingUserId());
    ...
    private void registerSettingsObserver() {
        final Uri enabledWeatherProviderServiceUri = EVSettings.Secure.getUriFor(
                EVSettings.Secure.WEATHER_PROVIDER_SERVICE);
        ContentObserver observer = new ContentObserver(BackgroundThread.getHandler()) {
            @Override
            public void onChange(boolean selfChange, Uri uri, int userId) {
                if (enabledWeatherProviderServiceUri.equals(uri)) {
                    String activeSrvc = EVSettings.Secure.getString(mContext.getContentResolver(),
                            EVSettings.Secure.WEATHER_PROVIDER_SERVICE);
                    disconnectClient();
                    if (activeSrvc != null) {
